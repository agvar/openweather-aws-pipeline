        ...
        try: 
            ...
            for zipcode in self.zipcodes:
                    ...
                    if not lat or not lon:
                        raise ValueError("Latitude or Longitude not received from url response :{response_geo_json}")
                    weather_params = {
                    "lat":lat,
                    "lon":lon,
                    'units': 'imperial',
                    ...
                    weather_response = apiManager.API_get(self.weather_url,weather_params,self.TIMEOUT)
                    weather_json_response = apiManager.API_parse_json(weather_response)
                    s3_response = s3Operations.store_object_in_s3(self.source_bucket,zipcode,json.dumps(weather_json_response))
